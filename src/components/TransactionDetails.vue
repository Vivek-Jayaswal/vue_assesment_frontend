                    ...
                    <tr v-for="(row, rowIndex) in rows" :key="rowIndex">
                        <td class="text-wrap" v-for="(heading, colIndex) in headings" :key="colIndex">
                            <template v-if="heading === 'Total'">
                                {{ calculateTotal(row, rowIndex) }}
                            </template>
                            <template v-else>
                                <input v-if="heading === 'Product'" type="text" v-model="row[heading]"
                                    :placeholder="`Enter ${heading}`" />
                                <input v-else type="number" v-model="row[heading]" :placeholder="`Enter ${heading}`"
                                    step="any" @input="updateRowTotal(rowIndex)" />
                            </template>
                        </td>
                        <td>
            ...
            <div class="lg:hidden relative">
                <div v-for="(row, rowIndex) in rows" :key="rowIndex"
                    ...
                    <div v-for="(heading, colIndex) in headings" :key="colIndex" class="flex justify-between">
                        ...
                        <template v-else>
                            <input v-if="heading === 'Product'" type="text" v-model="row[heading]"
                                class="border px-2 py-1 w-1/2" :placeholder="`Enter ${heading}`" />
                            <input v-else type="number" v-model="row[heading]" class="border px-2 py-1 w-1/2"
                                :placeholder="`Enter ${heading}`" step="any" @input="updateRowTotal(rowIndex)" />
                        </template>
                    </div>
...
export default {
    ...
    methods: {
        ...
        calculateTotal(row, rowIndex) {
            ...
            const total = (rate * quantity - discount).toFixed(2);
            this.rows[rowIndex].Total = total;
            return total;
        },
        updateRowTotal(rowIndex) {
            const row = this.rows[rowIndex];
            this.calculateTotal(row, rowIndex);
        },
        calculateGrandTotal() {
            return this.rows
                .reduce((total, row) => total + parseFloat(row.Total || 0), 0)
                .toFixed(2);
        },
        validateRows() {
